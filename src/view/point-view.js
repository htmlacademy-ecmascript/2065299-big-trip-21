...
function createPointTemplate({ point, pointDestination, pointOffers }) {
  ...
  return /*html*/ `
    <li class="trip-events__item">
      <div class="event">
        <time class="event__date" datetime="${formatToFullDate(
    dateFrom
  )}">${formatToDate(dateFrom)}</time>
    ...
    dateFrom
  )}">${formatToTime(dateFrom)}</time>
            &mdash;
            <time class="event__end-time" datetime="${formatToFullDate(
    dateTo
  )}">${formatToTime(dateTo)}</time>
          </p>
...
export default class PointView extends AbstractView {
  ...
  constructor({ point, pointDestination, pointOffers, onEditBtnClick }) {
    super();
    this.#point = point;
